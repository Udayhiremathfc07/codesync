(() => {
  ...
  socket.on("update_users", (data) => {
    ...
    for (const u in cursorWidgets) {
      if (!users.includes(u)) {
        try {
          editor.removeContentWidget(cursorWidgets[u]);
        } catch (e) {}
  ...
  require(["vs/editor/editor.main"], function () {
    ...
    if (restored) {
      ...
      localStorage.removeItem("restore_code");
    }
    ...
    editor.onDidChangeModelContent(() => {
      ...
      clearTimeout(t);
      t = setTimeout(() => {
        socket.emit("code_change", { room, code: editor.getValue() });
      }, 150);
  ...
  socket.on("run_result", (data) => {
    ...
    if (data.output?.run) {
      out.textContent =
        ...
        data.output.run.stdout ||
        data.output.run.stderr ||
        JSON.stringify(data.output, null, 2);
    } else if (data.error) {
      out.textContent = "Error: " + data.error;
    } else {
      out.textContent = "Unknown response: " + JSON.stringify(data);
    }
  });
  ...
  document.getElementById("sendBtn").addEventListener("click", () => {
    ...
    if (!msg) return;
    socket.emit("chat_message", { username, room, msg });
    input.value = "";
  });
  ...
  document.getElementById("leaveBtn").addEventListener("click", () => {
    ...
    sessionStorage.removeItem("codesync_username");
  ...
  dragY.addEventListener("mousedown", (e) => {
    ...
    document.onmousemove = (ev) => {
      const newHeight = window.innerHeight - ev.clientY - 50;
  ...
  dragX.addEventListener("mousedown", (e) => {
    e.preventDefault();
    document.onmousemove = (ev) => {
      chat.style.width = window.innerWidth - ev.clientX + "px";
    };
    document.onmouseup = () => (document.onmousemove = null);
  });
})();
